...
export async function handleRaftRequest(payload: any): Promise<PaymentExtensionResponse> {
  log.info(`Received request for paymentId=[${payload?.resource?.id}]`)

  if (validate(payload)) {
    const paymentObject = (payload?.resource as PaymentReference)?.obj
    const message = JSON.parse(paymentObject.custom?.fields?.request || '{}')
