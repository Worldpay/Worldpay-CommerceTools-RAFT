...
import { PaymentCreatedMessage, PaymentReference } from '@commercetools/platform-sdk'
...
describe('handleRaftRequest', () => {
  ...
  it('should return an error if the payload does not contain requestType', async () => {
    ...
    delete (payload?.resource as PaymentReference)?.obj?.custom?.fields?.request
  ...
  it('should return an empty update request', async () => {
    ...
    const returnActions = await handleRaftRequest({} as PaymentCreatedMessage)
