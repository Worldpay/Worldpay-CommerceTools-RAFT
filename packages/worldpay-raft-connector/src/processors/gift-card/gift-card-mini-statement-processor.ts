import { GiftCardMiniStatement, validateGiftCardMiniStatement } from '@gradientedge/worldpay-raft-messages'
...
const ENDPOINT = '/giftcard/ministatement'

export const GiftCardMiniStatementProcessor: Processor = {
  endpoint: ENDPOINT,
...
function validate(message: any): string[] {
  try {
    validateGiftCardMiniStatement(message)
  } catch (err) {
...
async function handleMessage(
  ...
  message: GiftCardMiniStatement,
): Promise<PaymentExtensionResponse> {
  log.debug(
    `Handle message for giftcardministatement with APITransactionID=${message?.giftcardministatement?.APITransactionID}`,
  )
  try {
