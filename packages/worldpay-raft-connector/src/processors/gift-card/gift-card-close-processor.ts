import { validateGiftCardClose } from '@gradientedge/worldpay-raft-messages'
...
const ENDPOINT = '/giftcard/close'

export const GiftCardCloseProcessor: Processor = {
  endpoint: ENDPOINT,
...
async function handleMessage(_transaction: Transaction | undefined, message: any): Promise<PaymentExtensionResponse> {
  log.debug(`Handle message for giftcardclose with APITransactionID=${message?.giftcardclose?.APITransactionID}`)
  try {
