import { GiftCardVirtualCard, validateGiftCardVirtualCard } from '@gradientedge/worldpay-raft-messages'
...
const ENDPOINT = '/giftcard/virtualcard'

export const GiftCardVirtualCardProcessor: Processor = {
  endpoint: ENDPOINT,
...
function validate(message: any): string[] {
  try {
    validateGiftCardVirtualCard(message)
  } catch (err) {
...
async function handleMessage(
  ...
  message: GiftCardVirtualCard,
): Promise<PaymentExtensionResponse> {
  log.debug(
    `Handle message for giftcardvirtualcard with APITransactionID=${message?.giftcardvirtualcard?.APITransactionID}`,
  )
  try {
