import { GiftCardStatusCard, validateGiftCardStatusCard } from '@gradientedge/worldpay-raft-messages'
...
async function handleMessage(
  ...
  message: GiftCardStatusCard,
): Promise<PaymentExtensionResponse> {
  log.debug(
    `Handle message for giftcardstatuscard with APITransactionID=${message?.giftcardstatuscard?.APITransactionID}`,
  )
  try {
